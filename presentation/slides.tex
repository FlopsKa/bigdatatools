...
\begin{figure}
  \centering
	\includegraphics[width=\textwidth,natwidth=826,natheight=462]{img/stream_analytics_intro_pipeline.png}
  \caption{ Bild von https://docs.microsoft.com/de-de/azure/stream-analytics/stream-analytics-introduction }
\end{figure}
\end{frame}
...
\end{figure}
  ...
  %nur auf unveränderlichen Daten ausgeführt.
	% \includegraphics[width=\textwidth]{img/swhistfig.png}
  %\begin{quote}
    %Eine Topologie ist ein „Berechnungsgraph“. Jeder Knoten im Graphen enthält
    %Verarbeitungslogik, die Kanten geben an, wie die Daten über die Knoten
    %verteilt werden.
  %\end{quote}

  %\begin{figure}[h]
    %\center
    %\scalebox{.4}{\input{./tikz/topology.tex}}
    %\caption{Übersicht über den Aufbau einer Topologie.}
    %\label{fig:topology}
  %\end{figure}
\end{frame}
...
\begin{frame}[t]{Word Count Beispiel}
  \begin{figure}[h]
    \center
    \scalebox{.8}{\input{./tikz/architecture.tex}}
